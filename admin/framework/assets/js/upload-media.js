(function ($) {
	...
	$.terminus_media_uploader = $.terminus_media_uploader || {};

	$.terminus_media_uploader.init = function () {

		$('.uploader-upload-button').unbind('click').on('click', function (e) {
			$.terminus_media_uploader.add(e);
		});

		$('.uploader-config-button').unbind('click').on('click', function (e) {
			$.terminus_media_uploader.upload_config(e);
		});

		$('body').unbind('click').on('click', '.uploader-remove-preview', function (e) {
			$.terminus_media_uploader.remove(e);
		});
	...
	$.terminus_media_uploader.add = function (e) {
		e.preventDefault();
	...
	$.terminus_media_uploader.remove = function (e) {
		e.preventDefault();
	...
	$.terminus_media_uploader.upload_config = function (e) {
		e.preventDefault();
					...
					success: function (response) {
						if (response.match('madImportConfig')) {
							...
							$('#mad-options-page').frameworkPopup({
								message: terminus_framework_localize.importsuccessOptions,
