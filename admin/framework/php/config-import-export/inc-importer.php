...
if ( !class_exists( 'WP_Import' ) ) {
	$class_wp_import = get_template_directory() . '/admin/framework/php/config-import-export/wordpress-importer.php';
...
if ( $importerError !== false ) {
	...
	if ( class_exists('WP_Import') ) {
		require_once( get_template_directory() . '/admin/framework/php/config-import-export/import-class.php' );
	}
	...
	if ( !is_file($default_path . '.xml') ) {
		...
		do_action('terminus_pre_import_hook');

		$wp_import = new terminus_wp_import();
		$wp_import->fetch_attachments = true;
		set_time_limit(0);
		$wp_import->import($default_path . '.xml');
		$wp_import->importSliders();
		$wp_import->save_settings($default_path . '.php');
		$wp_import->menu_install();

		do_action('terminus_after_import_hook');
