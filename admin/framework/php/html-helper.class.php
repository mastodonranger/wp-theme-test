...
if (!class_exists('TERMINUS_HTML_BUILD')) {

	class TERMINUS_HTML_BUILD extends TERMINUS_FRAMEWORK {
		...
		function hidden ($element) {
			...
			<div class="hidden-wrap">
				<input type="hidden" id="<?php echo esc_attr($element['id']) ?>" value="<?php echo esc_attr($element['std']) ?>" data-dependence="<?php echo esc_attr($element['dependence']) ?>" name="<?php echo esc_attr($element['id']) ?>">
			</div>
		...
		function upload ($element) {
			...
			$buttonText = esc_html__('Upload', 'terminus');
			...
			$placeholder = isset( $element['placeholder'] ) ? $element['placeholder'] : esc_html__('No media selected', 'terminus');

			ob_start(); ?>

			<div class="upload-wrap">
				<input type="hidden" class="data" data-mode="image">
				<input type="text" class="uploader-upload-input" placeholder="<?php echo esc_attr($placeholder) ?>" value="<?php echo esc_attr($image_url) ?>" name="<?php echo esc_attr($element['id']) ?>" id="<?php echo esc_attr($element['id']) ?>">
				<a href="#" title="<?php echo esc_attr($element['name']) ?>" data-title="<?php echo esc_attr($title) ?>" data-text="<?php echo esc_attr($buttonText) ?>" class="mad-option-button uploader-upload-button" id="<?php echo esc_attr($element['type'] . '_' . $element['id']) ?>"><?php echo esc_html($buttonText) ?></a>

				<div class="preview-thumbnail-container">
					<?php if ( preg_match('!\.jpg$|\.jpeg$|\.ico$|\.png$|\.gif$!', $image_url) ): ?>
						<a href="#" class="uploader-remove-preview"><?php esc_html_e('Remove', 'terminus'); ?></a>
						<img class="mad-option-image" src="<?php echo esc_url($image_url); ?>" alt="" />
					<?php endif; ?>
				</div>
		...
		function image_select($element) {

			if ( ! empty( $element['options'] ) ) {

				$output = '<div class="mad-table-container">';
                	$output .= '<ul class="mad-image-select">';

                		$x = 1;

						foreach ( $element['options'] as $k => $v ) {

							if ( ! is_array( $v ) ) { $v = array( 'img' => $v ); }
							if ( ! isset( $v['title'] ) ) { $v['title'] = ''; }
							if ( ! isset( $v['alt'] ) ) { $v['alt'] = $v['title']; }
							...
							$theValue = $k;
							$selected = ( checked( $element['std'], $theValue, false ) != '' ) ? ' mad-image-select-selected' : '';

							$output .= '<li class="mad-image-select">';
								$output .= '<label class="' . $selected . ' mad-image-select ' . $element['id'] . '_' . $x . '" for="' . $element['id'] . '_' . ( array_search( $k, array_keys( $element['options'] ) ) + 1 ) . '">';

									$output .= '<input type="radio" id="' . $element['id'] . '_' . ( array_search( $k, array_keys( $element['options'] ) ) + 1 ) . '" name="' . $element['id'] . '" value="' . $theValue . '" ' . checked( $element['std'], $theValue, false ) . '/>';
									$output .= '<img src="' . $v['img'] . '" alt="' . $v['alt'] . '" style="' . $style . '" />';

									$output .= '<span class="counter-label">'. $x .'</span>';
		...
		function section_description($element) {
			...
			<div class="mad-description">
				<?php echo sprintf('%s', $element['desc']); ?>
			</div><!--/ .mad-description-->
		...
		function page_header($option_pages) {
			...
			<div class="mad-admin-bar">
				<a class="help" target="_blank" href="http://velikorodnov.com/online-documentation/terminus/"><?php esc_html_e('Help', 'terminus'); ?></a>
				<a class="logout" href="<?php echo wp_logout_url(esc_url(home_url('/'))); ?>"><?php esc_html_e('Logout', 'terminus'); ?></a>
			</div><!--/ .mad-admin-bar-->
			...
			<aside class="mad-admin-aside">
				...
				<div id="mad-admin-logo">
					...
					<?php
						$title = $this->theme_data['title'];
						$version = $this->theme_data['version'];
						$prefix = substr($title, 0, 4);
						$title = substr($title, 4);
					?>

					<?php if ($title): ?>
						<a class="mad-admin-link" href="<?php echo admin_url('themes.php?page=terminus'); ?>">
		...
		function header_holder() {
			...
			<div class="mad-heading-holder">
				<div class="mad-heading">
					<h3 class="mad-heading-title"><?php echo strip_tags($title) . ' ' . esc_html__('Theme Options', 'terminus'); ?></h3>
				</div><!--/ .mad-heading-->
		...
		function footer_holder() {
			global $terminus_theme_data;
			$author = $terminus_theme_data['author'];

			ob_start(); ?>

			<li class="mad-footer-holder clearfix">
				<span>
					<?php esc_html_e('Copyright &copy; 2016.', 'terminus') ?>
					...
					<?php esc_html_e('All rights reserved.', 'terminus') ?>
				</span>
		...
		function reset_button() {
			return '<a href="#" class="button_reset mad-icon-reset">' . esc_html__('Reset Options', 'terminus') . '</a>';
		}

		function save_button() {
			return '<a href="#" class="button_save mad-icon-save">' . esc_html__('Save All Changes', 'terminus') . '</a>';
		}
		...
		function sitemap () {
			...
			<div class="sitemap-wrap">
				...
				<ul>
					<li>[terminus_wp_sitemap_page] <?php esc_html_e('To display a traditionnal sitemap', 'terminus'); ?></li>
					<li>[terminus_wp_sitemap_page only="page"] <?php esc_html_e('To display the pages', 'terminus'); ?></li>
					<li>[terminus_wp_sitemap_page only="page" sort="menu_order"] <?php printf(__('To display the pages sorted by menu order. Possible values are: %1$s.', 'terminus'), '\'post_title\', \'menu_order\', \'post_date\', \'post_modified\', \'ID\', \'post_author\', \'post_name\''); ?></li>
					<li>[terminus_wp_sitemap_page only="post"] <?php esc_html_e('To display the posts by category', 'terminus'); ?></li>
					<li>[terminus_wp_sitemap_page only="post" sort="count"] <?php printf(__('To display the posts by category. Categories sorted by number of posts. Possible values are: %1$s', 'terminus'), '\'id\', \'name\', \'slug\', \'count\', \'term_group\''); ?></li>
					<li>[terminus_wp_sitemap_page only="category"] <?php esc_html_e('To display the categories', 'terminus'); ?></li>
					<li>[terminus_wp_sitemap_page only="category" sort="count"] <?php printf(__('To display the categories sorted by number of posts. Possible values are: %1$s', 'terminus'), '\'id\', \'name\', \'slug\', \'count\', \'term_group\''); ?></li>
					<li>[terminus_wp_sitemap_page only="tag"] <?php esc_html_e('To display the tags', 'terminus'); ?></li>
					<li>[terminus_wp_sitemap_page only="archive"] <?php esc_html_e('To display the archives', 'terminus'); ?></li>
					<li>[terminus_wp_sitemap_page only="author"] <?php esc_html_e('To display the authors', 'terminus'); ?></li>
					<li>[terminus_wp_sitemap_page only="author" sort="post_count"] <?php printf(__('To display the authors, sorted by number of posts by author. Possible values are: %1$s.', 'terminus'), '\'name\', \'email\', \'url\', \'registered\', \'id\', \'user_login\', \'post_count\''); ?></li>
					<?php
					...
					foreach ( $post_types as $post_type ) :
						...
						?>
						<li>[terminus_wp_sitemap_page only="<?php echo esc_attr($cpt->name) ?>"] <?php printf(__('To display the %1$s', 'terminus'), strtolower($cpt->label)); ?></li>
					<?php endforeach; ?>
					...
					foreach ( $taxonomies_names as $taxonomy_name ) :
						...
						?>
						<li>[terminus_wp_sitemap_page only="<?php echo esc_attr($taxonomy_name); ?>"] <?php printf(esc_html__('To display the %1$s', 'terminus'), strtolower($taxonomy_label)); ?></li>
					<?php endforeach; ?>
					<li>[terminus_wp_sitemap_page display_title="false"] <?php esc_html_e('To display a traditionnal sitemap without the title', 'terminus'); ?></li>
				</ul>
			...
			<div class="import-wrap">
				<a class="button_import" <?php echo (isset($element['path'])) ? 'data-source="'. $element['source']. '"' : ''; ?> <?php echo (isset($element['path'])) ? 'data-path="'. $element['path']. '"' : ''; ?> href="#">
					...
					<?php if (isset($element['image'])): ?>
						<img src="<?php echo get_template_directory_uri() . '/' . $element['image']; ?>" alt=""/>
					<?php endif; ?>

					<div class="import-overlay">
						<span class="overlay-button"><?php esc_html_e('Click import data', 'terminus'); ?></span>
					</div><!--/ .import-overlay-->

				<span class="import-started">
					<span class="import-loading"></span>
					<strong><?php esc_html_e('Starting import.', 'terminus') ?></strong><br>
					<?php esc_html_e('Please dont reload the page. It may take a few minutes...', 'terminus') ?>
				</span><!--/ .import-started-->
				</a>
			</div><!--/ .import-wrap-->
		...
		function export_config_file() {
			...
			<span class="export-wrap">
				<a href="<?php echo admin_url('themes.php?page=terminus&theme_settings_export=1&generate_file=1'); ?>" class="button button-hero button_export">
					<?php esc_html_e('Export Theme Settings', 'terminus'); ?>
				</a>
			</span><!--/ .export-wrap-->
		...
		function import_config_file() {
			...
			<span class="export-wrap">
				<a href="javascript:void(0)" data-url="<?php echo admin_url("admin-ajax.php") ?>" data-_wpnonce="<?php echo esc_attr($nonce_import_config) ?>" data-action="<?php echo esc_attr($this->actionImportConfigOptions) ?>" class="button button-hero uploader-config-button">
					<?php esc_html_e('Import Theme Settings', 'terminus'); ?>
				</a>
			</span><!--/ .export-wrap-->
