...
if ( ! class_exists('Terminus_Envato_API') ) :
	...
	class Terminus_Envato_API {
		...
		/**
		 * Main Terminus_Envato_API Instance
		 *
		 * Ensures only one instance of this class exists in memory at any one time.
		 *
		 * @see Terminus_Envato_API()
		 * @uses Terminus_Envato_API::init_globals() Setup class globals.
		 * @uses Terminus_Envato_API::init_actions() Setup hooks and actions.
		 *
		 * @since 1.0.0
		 * @static
		 * @return object The one true Terminus_Envato_API.
		 * @codeCoverageIgnore
		 */
		 ...
		 *
		 * @see Terminus_Envato_API::instance()
		 *
		 * @since 1.0.0
		 * @access private
		 * @codeCoverageIgnore
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'terminus' ), '1.0.0' );
		}
		...
		private function init_globals() {
			// Envato API token.
			self::$token = Terminus()->registration->get_token();
		}
		...
		public function request( $url, $args = array() ) {
			$defaults = array(
				'headers' => array(
					...
					'User-Agent' => 'WordPress - Terminus ' . Terminus()->get_theme_version(),
				),
				'timeout' => 20,
			);
			...
			if ( empty( self::$token ) ) {
				return new WP_Error( 'api_token_error', __( 'An API token is required.', 'terminus' ) );
			}
			...
			if ( 200 !== $response_code && ! empty( $response_message ) ) {
				...
				return new WP_Error( $response_code, __( 'An unknown API error occurred.', 'terminus' ) );
			} else {
				$return = json_decode( wp_remote_retrieve_body( $response ), true );
				if ( null === $return ) {
					return new WP_Error( 'api_error', __( 'An unknown API error occurred.', 'terminus' ) );
				}
				return $return;
			}
		}
		...
		public function deferred_download( $id ) {
			...
			return add_query_arg( $args, esc_url( admin_url( 'admin.php?page=terminus' ) ) );
		}
