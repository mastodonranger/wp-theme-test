...
if (!function_exists('terminus_save_taxdata')) {

	function terminus_save_taxdata( $term_id, $ts_id, $taxonomy, $meta_boxes ) {
...
if (!function_exists('terminus_show_tax_add_meta_html')) {

	// Show Taxonomy Add meta
	function terminus_show_tax_add_meta_html($meta_box) {
		...
		<div class="terminus-form-field">
			...
			if ($type == "text") : // text ?>
				...
				<?php if ($desc) : ?><p><?php echo sprintf('%s', $desc) ?></p><?php endif; ?>
			<?php endif;
			...
			if ($type == "upload") :  ?>
				...
				<label for='upload_image'>
					...
					<button class="button_upload_image button" id="<?php echo esc_attr($name) ?>"><?php esc_html_e('Upload Image', 'terminus') ?></button>
					<button class="button_remove_image button" id="<?php echo esc_attr($name) ?>"><?php esc_html_e('Remove Image', 'terminus') ?></button>
				</label>
				<?php if ($desc) : ?><p><?php echo sprintf('%s', $desc) ?></p><?php endif; ?>
			<?php endif;
			...
			if ($type == "checkbox") : // checkbox ?>
				<label for="<?php echo esc_attr($name) ?>"><?php echo esc_html($title) ?></label>
				<label><input style="display:inline-block; width:auto;" type="checkbox" name="<?php echo esc_attr($name) ?>" value="<?php echo esc_attr($name) ?>" /> <?php echo sprintf('%s', $desc) ?></label>
			<?php endif;

			if (($type == 'radio') && (!empty($options))) :  ?>
				<label for="<?php echo esc_attr($name) ?>"><?php echo esc_html($title) ?></label>
				<?php foreach ($options as $key => $value) : ?>
					<input style="display:inline-block; width:auto;" type="radio" id="<?php echo esc_attr($name) ?>_<?php echo esc_attr($key) ?>" name="<?php echo esc_attr($name) ?>"  value="<?php echo esc_attr($name) ?>"/>
...
if (!function_exists('terminus_show_tax_edit_meta_html')) {

	// Show Taxonomy Edit meta
	function terminus_show_tax_edit_meta_html($term, $taxonomy, $meta_box) {
		...
		<tr class="terminus-form-field">
			...
			if ($type == "text") : // text ?>
				...
				<td>
					...
					<?php if ($desc) : ?><p class="description"><?php echo sprintf('%s', $desc) ?></p><?php endif; ?>
				</td>
			<?php endif;
			...
			if ($type == "checkbox") : ?>
				...
				<th scope="row" valign="top"><label for="<?php echo esc_attr($name) ?>"><?php echo esc_html($title) ?></label></th>
				<td>
					<label><input type="checkbox" name="<?php echo esc_attr($name) ?>" value="<?php echo esc_attr($name) ?>" <?php echo esc_attr($checked) ?> /> <?php echo sprintf('%s', $desc) ?></label>
				</td>
			<?php endif;
