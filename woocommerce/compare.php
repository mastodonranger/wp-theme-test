...
<head>
    ...
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Droid+Sans%3A400%2C700%7CDroid+Serif%3A400%2C700%2C400italic%2C700italic&#038;subset=latin%2Clatin-ext" />
    <link rel="stylesheet" href="<?php echo esc_url($this->stylesheet_url()) ?>" type="text/css" />
	<link rel="stylesheet" href="<?php echo get_template_directory_uri() ?>/css/terminus_icons.css" />
    <link rel="stylesheet" href="<?php echo get_template_directory_uri() ?>/config-plugins/plugins/compare/css/colorbox.css"/>
    <link rel="stylesheet" href="<?php echo get_template_directory_uri() ?>/config-plugins/plugins/compare/css/jquery.dataTables.css"/>
...
<h1 class="compare-title">
	...
	<?php if ( ! $is_iframe ) : ?><a class="close" href="javascript:void(0)"><?php esc_html_e( 'Close window [X]', 'terminus' ) ?></a><?php endif; ?>
</h1>
...
<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php esc_html_e( 'No products added in the compare table.', 'terminus' ) ?></td>
        </tr>
    ...
    <?php else : ?>
                ...
                <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->get_id(); ?>
                    <td class="<?php echo esc_attr($product_class); ?>"><?php
                        switch( $field ) {
                            ...
                            case 'image':
                                echo '<div class="image-wrap"><a target="_blank" href="'. esc_url(get_the_permalink($product->get_id())) . '">' . wp_get_attachment_image( $product->fields[$field], 'yith-woocompare-image' ) . '</a></div>';
                                break;

                            case 'add-to-cart':
                                woocommerce_template_loop_add_to_cart();
								echo '<div class="remove">';
									echo '<a href="' . esc_url(add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->get_id() ) )) . '" data-product_id="' . $product->get_id() .'">'. esc_html__( 'Remove', 'terminus' ) .'</a>';
								echo '</div>';
                               	break;
                            default:
        ...
        <?php if ( $repeat_price == 'yes' && isset( $fields['price'] ) ) : ?>
            <tr class="price repeated">
                <th><?php echo sprintf('%s', $fields['price']) ?></th>

                <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->get_id() ?>
                    <td class="<?php echo esc_attr($product_class) ?>"><?php echo sprintf('%s', $product->fields['price']) ?></td>
                <?php endforeach; ?>
        ...
        <?php if ( $repeat_add_to_cart == 'yes' && isset( $fields['add-to-cart'] ) ) : ?>
            <tr class="add-to-cart repeated">
                <th><?php echo sprintf('%s', $fields['add-to-cart']) ?></th>
